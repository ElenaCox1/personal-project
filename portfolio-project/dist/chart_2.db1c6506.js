...
var colorScale = d3.scaleOrdinal().range(['#E1EF85', '#F7D4EC']);
var xPositionScale = d3.scalePoint().range([0, width]);
d3.csv(require('/data/Cook_Accuracy_counts.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.selectAll('.graph').data(nested).enter().append('g').attr('transform', function (d) {
    ...
    container.selectAll('path').data(pie(datapoints)).enter().append('path').attr('d', function (d) {
      ...
      return colorScale(d.data.RESULT);
    });
    container.append('text').text(function (d) {
      return d.key;
    }).attr('x', xPositionScale(d.YEAR)).attr('y', height / 3).attr('font-size', 15).attr('fill', 'black').attr('text-anchor', 'middle');
  });
} // g.append('text')
// .text(d => d.key)
...
},{"d3":"../../../node_modules/d3/index.js","/data/Cook_Accuracy_counts.csv":"data/Cook_Accuracy_counts.csv"}]},{},["scripts/portfolio_2/chart_2.js"], null)
//# sourceMappingURL=/chart_2.db1c6506.js.map